...
If an Ethereum address with a 0x prefix is passed, the verified source code from Etherscan will be used. For example
    ...
    .addOption(new commander_1.Option('-d, --depth <value>', 'depth of connected classes to the base contracts. 1 will only show directly connected contracts, interfaces, libraries, structs and enums.').default('-1', 'all'))
...
program
    ...
    try {
        ...
        const structSlots = [];
        const slots = converterClasses2Slots_1.convertClasses2Slots(combinedOptions.contractName || contractName, filteredUmlClasses, structSlots);
        if (regEx_1.isAddress(fileFolderAddress)) {
            slots.address = fileFolderAddress;
        }
        debug(slots);
        const dotString = converterSlots2Dot_1.convertAllSlots2Dot(slots, structSlots);
        await writerFiles_1.writeOutputFiles(dotString, fileFolderAddress, combinedOptions.outputFormat, combinedOptions.outputFileName);
    }
    catch (err) {
