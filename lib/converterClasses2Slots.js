...
exports.isElementary = exports.calcStorageByteSize = exports.parseStructSlots = exports.convertClasses2Slots = exports.StorageType = void 0;
...
let slotId = 0;
const convertClasses2Slots = (contractName, umlClasses, structSlots = []) => {
 ...
 */
const parseStorage = (umlClass, umlClasses, storages, structSlots) => {
    ...
    umlClass.attributes.forEach((attribute) => {
        ...
        const linkedStructSlots = exports.parseStructSlots(attribute, umlClasses, structSlots);
        const structSlotsId = linkedStructSlots === null || linkedStructSlots === void 0 ? void 0 : linkedStructSlots.id;
...
const parseStructSlots = (attribute, otherClasses, structSlots) => {
    if (attribute.attributeType === umlClass_1.AttributeType.UserDefined) {
        // Have we already created the structSlots?
        const existingStructSlot = structSlots.find((dep) => dep.name === attribute.type);
        ...
        if (dependentClass.stereotype === umlClass_1.ClassStereotype.Struct) {
            const storages = parseStorage(dependentClass, otherClasses, [], structSlots);
    ...
    if (attribute.attributeType === umlClass_1.AttributeType.Mapping ||
        ...
        if (result !== null && result[1] && !exports.isElementary(result[1])) {
            // Find UserDefined type
            const typeClass = otherClasses.find(({ name }) => name === result[1]);
            if (!typeClass) {
            ...
            if (typeClass.stereotype === umlClass_1.ClassStereotype.Struct) {
                const storages = parseStorage(typeClass, otherClasses, [], structSlots);
                const newStructSlots = {
                    id: slotId++,
                    name: typeClass.name,
                    type: StorageType.Struct,
...
exports.parseStructSlots = parseStructSlots;
        ...
        switch (attributeClass.stereotype) {
            ...
            case umlClass_1.ClassStereotype.Struct:
                ...
                attributeClass.attributes.forEach((structAttribute) => {
                    ...
                    const attributeSize = exports.calcStorageByteSize(structAttribute, umlClass, otherClasses);
...
//# sourceMappingURL=converterClasses2Slots.js.map