 ...
 */
const parseStorage = (umlClass, umlClasses, storages, dependencies) => {
...
const calcStorageByteSize = (attribute, umlClass, otherClasses) => {
    ...
    if (attribute.attributeType === umlClass_1.AttributeType.UserDefined) {
        ...
        const size = calcStructOrEnumByteSize(attribute.type, umlClass, otherClasses);
        if (size) {
            return size;
        }
...
const calcStructOrEnumByteSize = (typeName, umlClass, otherClasses) => {
    ...
    for (const parent of parentContracts) {
        const parentClass = otherClasses.find((umlClass) => umlClass.name === parent.targetUmlClassName);
        if (!parentClass)
            throw Error(`Failed to find parent contract ${parent.targetUmlClassName} of ${umlClass.name}`);
        // recursively see if the User Defined type is in an inherited contract
        const size = calcStructOrEnumByteSize(typeName, parentClass, otherClasses);
        if (size) {
            return size;
        }
