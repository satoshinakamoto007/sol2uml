...
const parseUmlClassesFromFiles = async (filesOrFolders, ignoreFilesOrFolders, depthLimit = -1) => {
    ...
    for (const file of files) {
        ...
        const umlClass = converterAST2Classes_1.convertAST2UmlClasses(node, relativePath, true);
...
function parseSolidityFile(fileName) {
    let solidityCode;
    try {
        solidityCode = fs_1.readFileSync(fileName, 'utf8');
    }
    catch (err) {
        throw new verror_1.VError(err, `Failed to read solidity file ${fileName}.`);
    }
    try {
        return parser_1.parse(solidityCode, {});
    }
    catch (err) {
        throw new verror_1.VError(err, `Failed to parse solidity code in file ${fileName}.`);
    }
}
