...
function parseContractDefinition(umlClass, node) {
    ...
    node.subNodes.forEach((subNode) => {
        if (parserTypes_1.isStateVariableDeclaration(subNode)) {
        ...
        else if (parserTypes_1.isUsingForDeclaration(subNode)) {
        ...
        else if (parserTypes_1.isFunctionDefinition(subNode)) {
        ...
        else if (parserTypes_1.isModifierDefinition(subNode)) {
            umlClass.operators.push({
        ...
        else if (parserTypes_1.isEventDefinition(subNode)) {
            umlClass.operators.push({
        ...
        else if (parserTypes_1.isStructDefinition(subNode)) {
            let structMembers = [];
        ...
        else if (parserTypes_1.isEnumDefinition(subNode)) {
            let enumValues = [];
            subNode.members.forEach((member) => {
...
function parseTypeName(typeName) {
    ...
    switch (typeName.type) {
        ...
        case 'Mapping':
            return ('mapping\\(' + ((_a = typeName.keyType) === null || _a === void 0 ? void 0 : _a.name) ||
